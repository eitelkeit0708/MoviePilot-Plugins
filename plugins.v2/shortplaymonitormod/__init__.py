...
class ShortPlayMonitorMod(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._enabled or self._onlyonce:
            ...
            # 运行一次定时服务
            if self._onlyonce:
                logger.info("短剧监控Mod服务启动，立即运行一次")
                ...
                if is_directory:
                    ...
                    try:
                        ...
                        if matches:
                            ...
                            # 尝试匹配仅有季数的情况（如 S01），自动补充 E01
                            season_pattern = r'S(\d+)'
                            season_match = re.search(season_pattern, Path(target_path).name)
                            if season_match:
                                season_num = season_match.group(1)
                                episode_tag = f"S{season_num}E01"
                                logger.info(f"文件 {Path(event_path).name} 未找到集数，自动补充为 {episode_tag}")
                                target_path = Path(
                                    target_path).parent / f"{episode_tag}{Path(Path(target_path).name).suffix}"
                            else:
                                # 完全没有季集信息，默认 S01E01
                                episode_tag = "S01E01"
                                logger.info(f"文件 {Path(event_path).name} 未找到季数和集数，自动补充为 {episode_tag}")
                                target_path = Path(
                                    target_path).parent / f"{episode_tag}{Path(Path(target_path).name).suffix}"
                    except Exception as e:
                        logger.error(f"媒体重命名异常: {str(e)}")
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'info',
                                            'variant': 'tonal',
                                            'text': 'Mod版特性：不含集数的合并短剧文件将自动补充E01编号。'
